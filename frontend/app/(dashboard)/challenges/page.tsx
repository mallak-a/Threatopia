      ...
      {loading ? (
        ...
        >
          {filteredChallenges.map((challenge) => {
            const normalizedCategory = challenge.category.replace('_', '-') as ChallengeCategory
            const CategoryIcon = categoryIcons[normalizedCategory] || Shield
            const categoryColor = categoryColors[normalizedCategory] || 'bg-primary/10 text-primary border-primary/30'
                  ...
                  <Card className={`h-full glass border-primary/10 hover:neon-glow-sm transition-all duration-300 cursor-pointer group relative overflow-hidden ${isCompleted ? 'border-green-500/30' : ''}`}>
                    ...
                    <CardContent className="p-5">
                      ...
                      <div className="flex items-start gap-3 mb-4">
                        <div className={`p-2.5 rounded-lg ${categoryColor.split(' ').slice(0, 1).join(' ')}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground capitalize">
                            {challenge.category.replace('-', ' ').replace('_', ' ')}
                          </p>
                        </div>
                      </div>
